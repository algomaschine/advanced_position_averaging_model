...
class AdvancedPositionAveraging:
    ...
    def __init__(self, minute_data_path, initial_capital=1000000, start_date='2025-01-01'):
        ...
        self.param_space = {
            ...
            'hurst_period': [50, 100, 200],              # Hurst calculation window
            'use_dynamic_periods': [False]                # Enable/disable dynamic periods (optional)
        }
    ...
    def calculate_technical_indicators(self, data, params):
        ...
        hurst_period = params.get('hurst_period', 100)  # Default 100 bars (about 100 minutes)
        data['hurst_exponent'] = self._calculate_hurst_exponent_rolling(data['close'], hurst_period)
        
        # Market regime classification
        data['market_regime'] = data['hurst_exponent'].apply(self._classify_market_regime)
        
        # Dynamic period adjustment (like non-lag MA)
        if params.get('use_dynamic_periods', False):
            data = self._apply_dynamic_periods(data, params)
        
        return data
    
    def _calculate_hurst_exponent_rolling(self, prices, period=100):
        """Calculate rolling Hurst exponent using R/S analysis (optimized)."""
        hurst_values = pd.Series(index=prices.index, dtype=float)
        hurst_values[:] = 0.5  # Default to random walk
        
        # Only calculate every 10 bars for performance (or use last value for each day)
        step = max(1, period // 10)  # Calculate every 10th of the period
        ...
        for i in range(period, len(prices), step):
            ...
            try:
                ...
                # Forward fill until next calculation
    ...
    def _calculate_hurst_exponent(self, prices):
        ...
        # Convert to log returns for better stability
        ...
        n = len(log_returns)
        max_lag = min(n // 4, 50)  # Limit max lag to avoid instability
    ...
    def _classify_market_regime(self, hurst):
        ...
        if hurst > 0.55:
            ...
            return 'weak_mean_reverting'  # Weak mean reversion
        else:
            return 'neutral'  # Random walk / neutral
    
    def _apply_dynamic_periods(self, data, params):
        """Apply dynamic period adjustment like non-lag MA indicators."""
        # Calculate adaptive period based on volatility
        volatility = data['returns'].rolling(50).std()
        base_period = params.get('bb_period', 20)
        
        # Adaptive period: higher vol = shorter period, lower vol = longer period
        vol_percentile = volatility.rolling(100).quantile(0.5)
        adaptive_factor = 1.0 / (1.0 + volatility / vol_percentile)
        adaptive_period = (base_period * adaptive_factor).clip(lower=base_period*0.5, upper=base_period*1.5).astype(int)
        
        # Recalculate BB with adaptive periods
        data['bb_period_dynamic'] = adaptive_period
        # Note: Full adaptive BB recalculation would be expensive, so we use approximation
        data['bb_period_used'] = adaptive_period
    ...
    def _detect_daily_signals(self, day_data, strategy_type, params):
        """Detect signals for a single day with parameters and market regime awareness."""
        signals = []
        ...
        # Get market regime for strategy weighting
        if 'market_regime' in day_data.columns and len(day_data) > 0:
            current_regime = day_data['market_regime'].iloc[-1]
            hurst_value = day_data['hurst_exponent'].iloc[-1] if 'hurst_exponent' in day_data.columns else 0.5
        else:
            current_regime = 'neutral'
            hurst_value = 0.5
        
        # Calculate strategy weights based on market regime
        regime_weights = self._get_regime_strategy_weights(current_regime, hurst_value)
        
        # Strategy 1: Breakout signals (weighted by regime)
        breakout_signals = self._detect_breakout_signals(day_data, strategy_type, base_confidence, params)
        # Apply regime-based confidence adjustment
        for signal in breakout_signals:
            signal['confidence'] *= regime_weights['breakout']
        signals.extend(breakout_signals)
        
        # Strategy 2: Mean reversion signals (weighted by regime)
        ...
        for signal in mean_reversion_signals:
            signal['confidence'] *= regime_weights['mean_reversion']
        signals.extend(mean_reversion_signals)
        
        # Strategy 3: Momentum signals (weighted by regime)
        momentum_signals = self._detect_momentum_signals(day_data, strategy_type, base_confidence, params)
        # Apply regime-based confidence adjustment
        for signal in momentum_signals:
            signal['confidence'] *= regime_weights['momentum']
        signals.extend(momentum_signals)
        
        # Strategy 4: Support/Resistance signals (weighted by regime)
        sr_signals = self._detect_support_resistance_signals(day_data, strategy_type, base_confidence, params)
        # Apply regime-based confidence adjustment
        for signal in sr_signals:
            signal['confidence'] *= regime_weights['support_resistance']
        signals.extend(sr_signals)
        
        return signals
    
    def _get_regime_strategy_weights(self, regime, hurst_value):
        """Get strategy weights based on market regime.
        
        Returns weights for each strategy type based on Hurst exponent:
        - Trending markets (H > 0.5): Favor momentum and breakout
        - Mean-reverting markets (H < 0.5): Favor mean reversion and S/R
        - Neutral markets (H ≈ 0.5): Balanced weights
        """
        if regime == 'trending':
            # Strong trending: high momentum/breakout, low mean reversion
            return {
            ...
            # Strong mean reversion: high mean reversion/S/R, low momentum
            return {
                'breakout': 0.4,
                'momentum': 0.3,
                'mean_reversion': 1.6,
